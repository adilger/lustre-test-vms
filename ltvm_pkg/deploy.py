...
def configure_test_disks(
    ...
    # Rewrite the block where it already sits rather than deleting it and
    # appending.  cfg/local.sh is sourced, so the last assignment wins:
    # re-appending moves this block past anything set after it, which is
    # how an llmount silently put the OSTs back on the virtio disks after
    # configure_ram_osts had repointed them at brd.
    script = (
        f"printf '# --- VM disk configuration (generated by ltvm deploy) ---\\n"
        f"{snippet}\\n"
        f"# --- END VM disk configuration ---\\n' > {tmp}; "
        f"if grep -q '^# --- VM disk configuration' {cfg} 2>/dev/null; then "
        f"awk -v f={tmp} '"
        f"/^# --- VM disk configuration/ "
        f"{{ while ((getline l < f) > 0) print l; close(f); s=1; next }} "
        f"/^# --- END VM disk configuration/ {{ s=0; next }} "
